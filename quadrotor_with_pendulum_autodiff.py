...
class QuadrotorPendulum(VectorSystem):
    def __init__(self, Q, R, Qf, mb = 1., lb = 0.5, 
        ...
        self.mb = mb if isinstance(mb, AutoDiffXd) else AutoDiffXd(mb)
        self.lb = lb if isinstance(lb, AutoDiffXd) else AutoDiffXd(lb)
        self.m1 = m1 if isinstance(m1, AutoDiffXd) else AutoDiffXd(m1)
        self.l1 = l1 if isinstance(l1, AutoDiffXd) else AutoDiffXd(l1)
        self.g = g if isinstance(g, AutoDiffXd) else AutoDiffXd(g)
        self.input_max = input_max if isinstance(input_max, AutoDiffXd) else AutoDiffXd(input_max)
...
#     return (M, C, tauG, B)
    ...
    def GetManipulatorDynamics(self, q, qd):
        # Dynamics matrices using PyDrake's mathematical operations
        M = np.array([[self.mb + self.m1, 0., 0., self.m1 * self.l1 * cos(q[3])],
                    [0., self.mb + self.m1, 0., self.m1 * self.l1 * sin(q[3])],
                    [0., 0., self.Ib, 0.],
                    [self.m1 * self.l1 * cos(q[3]), self.m1 * self.l1 * sin(q[3]), 0., self.I1 + self.m1 * self.l1**2]])

        C = np.array([[0., 0., 0., -self.m1 * self.l1 * sin(q[3]) * qd[3]],
                    [0., 0., 0., self.m1 * self.l1 * cos(q[3]) * qd[3]],
                    [0., 0., 0., 0.],
                    [0., 0., 0., 0.]])
...
#     return (A, B)
    ...
    def GetLinearizedDynamics(self, u_f, x_f):
        ...
        # Ensure that alpha and I are of AutoDiffXd type
        alpha = self.m1 * self.l1 / (self.m1 + self.mb)
        if not isinstance(alpha, AutoDiffXd):
            alpha = AutoDiffXd(alpha)

        I = self.I1 + self.m1 * self.mb * self.l1**2 / (self.m1 + self.mb)
        if not isinstance(I, AutoDiffXd):
            I = AutoDiffXd(I)

        A = np.zeros((8, 8), dtype=AutoDiffXd)
        B = np.zeros((8, 2), dtype=AutoDiffXd)

        # Fill in the matrices using PyDrake's sin, cos functions
        B[4,0] = -1 / (self.m1 + self.mb) * sin(q[2]) + alpha**2 / I * cos(q[3]) * sin(q[3] - q[2])
        B[4,1] = B[4,0]
        ...
        A[:4,4:] = np.diag(np.ones(4, dtype=AutoDiffXd))
        ...
        # Define the C matrix using PyDrake's sin, cos functions and AutoDiffXd
        C[4,0] = alpha * sin(q[3]) * qd[3]**2 + (-1 / (self.m1 + self.mb) * sin(q[2]) + alpha**2 / I * cos(q[3]) * sin(q[3] - q[2])) * (u_f[0] + u_f[1])
        C[5,0] = -alpha * cos(q[3]) * qd[3]**2 - self.g + ((self.I1 + self.m1 * self.l1**2) / (self.m1 + self.mb) / I * cos(q[2]) - alpha**2 / I * cos(q[3]) * cos(q[3] - q[2])) * (u_f[0] + u_f[1])
        C[6,0] = (-u_f[0] + u_f[1]) / self.Ib
        C[7,0] = -alpha / I * sin(q[3] - q[2]) * (u_f[0] + u_f[1])
...
#     return A_d, B_d
    ...
    def discrete_time_linearized_dynamics(self, T):
        # Ensure T is AutoDiffXd compatible
        T_ad = AutoDiffXd(T)
        ...
        # Discretize the linearized dynamics
        I = np.eye(len(x_f), dtype=AutoDiffXd)  # Identity matrix with AutoDiffXd type
        A_d = I + A_c * T_ad
        B_d = B_c * T_ad
    ...
    def x_d(self):
        # Nominal state with AutoDiffXd type
        return np.array([AutoDiffXd(5), AutoDiffXd(2), AutoDiffXd(0), AutoDiffXd(0),
                         AutoDiffXd(0), AutoDiffXd(0), AutoDiffXd(0), AutoDiffXd(0)], dtype=AutoDiffXd)
    ...
    def u_d(self):
        # Nominal input with AutoDiffXd type
        half_gravity = AutoDiffXd((self.mb + self.m1) * self.g / 2)
        
        return np.array([half_gravity, half_gravity], dtype=AutoDiffXd)
